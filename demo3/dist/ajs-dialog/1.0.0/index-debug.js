define("ajs-dialog/1.0.0/index-debug", ["jquery", "transit/0.9.9/jquery.transit-debug"], function(require, exports, module) {
  'use strict';
  var dialog = require("ajs-dialog/1.0.0/dialog-plus-debug");
  var $ = require('jquery');
  require("transit/0.9.9/jquery.transit-debug");
  ...
  function Dialog(opt) {
    ...
    return dialog(conf, undefined, function() {
      if (this.modal && this.lock !== false) {
        html.setAttribute('style', '');
      }
    });
  }
  Dialog.tip = function(opt) {
    ...
    var conf = {
      ...
      onbeforeshow: function() {
        ...
        var self = this;
        var $node = $(this.node);
        $node.css('opacity', 0).transition({
          y: (-this.node.offsetHeight + 30) + 'px'
        }, 10, function() {
          $node.css('opacity', 1).transition({
            y: '43px'
          });
          setTimeout(function() {
  ...
  Dialog.confirm = function(opt, yes, no) {
    var html = document.getElementsByTagName('body')[0];
    var conf = {
      title: '警告',
      cancel: false,
      ...
      button: [{
        id: 'yes',
        value: '好',
        callback: typeof yes == 'function' ? yes : Dialog.noop,
        classname: 'button button-primary'
      }, {
        id: 'no',
        value: '取消',
        callback: typeof no == 'function' ? no : Dialog.noop,
        classname: 'button'
      }]
    };
    for (var i in opt) {
  ...
  Dialog.noop = function() {};
  module.exports = Dialog;
});
define("ajs-dialog/1.0.0/dialog-plus-debug", ["jquery"], function(require, exports, module) {
  /*!
   ...
   */
  var $ = require('jquery');
  var dialog = require("ajs-dialog/1.0.0/dialog-debug");
  var drag = require("ajs-dialog/1.0.0/drag-debug");
  dialog.oncreate = function(api) {
    var options = api.options;
    var originalOptions = options._;
    var allowDrag = options.allowDrag;
    // 页面地址
    var url = options.url;
    // 页面加载完毕的事件
...
define("ajs-dialog/1.0.0/dialog-debug", ["jquery"], function(require, exports, module) {
  /*!
   * artDialog v6.0.0
   * Date: 2013-12-25
   * https://github.com/aui/artDialog
   ...
   */
  var $ = require('jquery');
  var Popup = require("ajs-dialog/1.0.0/popup-debug");
  var defaults = require("ajs-dialog/1.0.0/dialog-config-debug");
  ...
  $.extend(prototype, {
    ...
    button: function(args) {
      ...
      if (typeof args === 'string') {
        ...
        $.each(args, function(i, val) {
          ...
          val.id != 'cancel' && (html += '<button' + ' type="button"' + ' data-id="' + val.id + '"' + (val.classname ? ' class="' + val.classname + (val.autofocus ? ' ui-dialog-autofocus' : '') + '"' : '') + (val.disabled ? ' disabled' : '') + (val.autofocus ? ' autofocus ' + (!val.classname ? 'class="ui-dialog-autofocus"' : '') : '') + '>' + val.value + '</button>');
        });
      }
    ...
    _trigger: function(id) {
      ...
      return typeof fn !== 'function' || fn.call(this) !== false ? (id == 'cancel' ? this.close().remove() : this._trigger('cancel')) : this;
    },
    // 关闭
    toClose: function() {
...
define("ajs-dialog/1.0.0/popup-debug", ["jquery"], function(require, exports, module) {
  /*!
   * popupjs
   * Date: 2014-01-15
  ...
  $.extend(Popup.prototype, {
    ...
    show: function(anchor) {
      ...
      this.__dispatchEvent('beforeshow');
    ...
    __lock: function() {
      ...
      backdrop.css(backdropCss).insertAfter(popup).animate({
    ...
    __unlock: function(time) {
      if (this.modal && this.lock !== false) {
        ...
        backdrop.animate({
          ...
          popup.removeClass(this.className + '-modal');
          backdrop.remove();
          delete this.modal;
        });
      }
...
define("ajs-dialog/1.0.0/dialog-config-debug", [], function(require, exports, module) {
  /*!
...
define("ajs-dialog/1.0.0/drag-debug", ["jquery"], function(require, exports, module) {
  /*!
   * drag.js
   * Date: 2013-12-06
   * (c) 2009-2013 TangBin, http://www.planeArt.cn
